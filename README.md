...
解析には、ある程度のサンプル数が必要なため場合によってはしばらく表示されないかもしれないですが気長にお待ちください。また、共起ネットワークおよびサンバーストプロットに関しては、ある程度の長さのコメントが増えないと計算ができないため、おそらく頻度のプロットおよびワードクラウドより出るのが遅くなると思われます。
